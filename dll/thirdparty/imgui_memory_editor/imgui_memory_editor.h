...
// - v0.55 (2024/08/19): added BgColorFn to allow setting background colors independently from highlighted selection.
// (#27) [@StrikerX3]
//                       added MouseHoveredAddr public readable field. (#47, #27) [@StrikerX3]
//                       fixed a data preview crash with 1.91.0 WIP. fixed contiguous highlight color when using data
//                       preview. *BREAKING* added UserData field passed to all optional function handlers: ReadFn,
...
// - PageUp/PageDown are not supported because we use _NoNav. This is a good test scenario for working out idioms of how
// to mix natural nav and our own...
...
struct MemoryEditor {
	...
	bool (*HighlightFn)(const ImU8 *mem, size_t off,
						void *user_data);  // = 0      // optional handler to return Highlight property (to support
										   // non-contiguous highlighting).
	ImU32 (*BgColorFn)(
	...
	void DrawContents(void *mem_data_void, size_t mem_size, size_t base_display_addr = 0x0000) {
		...
		if (OptShowAscii)
			draw_list->AddLine(ImVec2(window_pos.x + s.OffsetAsciiMinX - s.GlyphWidth, window_pos.y),
							   ImVec2(window_pos.x + s.OffsetAsciiMinX - s.GlyphWidth, window_pos.y + 9999),
				 ...
				 line_i++)	// display only visible lines
			{
				...
				if (OptShowAscii) {
					...
					const size_t mouse_addr =
						(mouse_off_x >= 0.0f && mouse_off_x < s.OffsetAsciiMaxX - s.OffsetAsciiMinX)
	...
	void DrawPreviewLine(const Sizes &s, void *mem_data_void, size_t mem_size, size_t base_display_addr) {
		...
		static const ImGuiDataType supported_data_types[] = {
			ImGuiDataType_S8,  ImGuiDataType_U8,  ImGuiDataType_S16, ImGuiDataType_U16,	  ImGuiDataType_S32,
	...
	void DrawPreviewData(size_t addr, const ImU8 *mem_data, size_t mem_size, ImGuiDataType data_type,
		...
		if (ReadFn)
			for (int i = 0, n = (int)size; i < n; ++i) buf[i] = ReadFn(mem_data, addr + i, UserData);
		else
