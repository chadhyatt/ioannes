...
// Custom ImGui widgets (definitely didn't yoink from the demo src)
...
struct ConsoleLog {
	...
	void Draw() {
		...
		// Options menu
		if (ImGui::BeginPopup("id_console_log_opts")) {
			ImGui::Checkbox("Auto-scroll", &AutoScroll);
			ImGui::EndPopup();
		}

		// Main window
		if (ImGui::Button("Options")) ImGui::OpenPopup("id_console_log_opts");
		ImGui::SameLine();
		...
		if (ImGui::BeginChild("scrolling", ImVec2(0, 0), ImGuiChildFlags_None, ImGuiWindowFlags_HorizontalScrollbar)) {
			if (clear) Clear();
			if (copy) ImGui::SetClipboardText(Buf.c_str());	 // ImGui::LogToClipboard();
...
typedef struct {  // WindowState
	...
	// For proxy (Go) side, doesn't run in the GUI thread
	...
	// Packet log
	bool isCapturing;
	std::vector<ProxyPacket *> packets;
	MemoryEditor hexDumpEditor;

} WindowState;

extern WindowState *wndState;
