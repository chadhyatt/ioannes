...
local function bitBuffer(stream, opts)
    if stream ~= nil then
    ...
    local function dumpString()
        ...
        for i = 1, byteCount, 4096 do -- groups of 4096 bytes is the point at which there are diminishing returns
            output[c] = string.char(unpack(bytes, i, math.min(byteCount, i + 4095)))
            c = c + 1
        end
    ...
    local function dumpBase64()
        ...
        local k = 1
        for i = 1, c, 0x1000 do
            realOutput[k] = string.char(unpack(output, i, math.min(c, i + 0xfff)))
            k = k + 1
        end
    ...
    local function exportChunk(chunkLength)
        ...
        return coroutine.wrap(function()
            ...
            for i = 1, byteCount, chunkLength do
                local chunk = string.char(unpack(bytes, i, math.min(byteCount, i + realChunkLength)))
    ...
    local function exportBase64Chunk(chunkLength)
        ...
        return coroutine.wrap(function()
            ...
            for i = 1, c, chunkLength do
                local chunk = string.char(unpack(output, i, math.min(c, i + realChunkLength)))
    ...
    local function writeBits(...)
        ...
        for _, v in ipairs(packed) do
            ...
            lastByte = lastByte +
                (v == 1 and powers_of_2[7 - bits] or 0) -- Add the current bit to lastByte, from right to left
            bits = bits + 1
    ...
    local function writeByte(n)
        ...
        if bits == 0 then
            ...
            bytes[byteCount] = lastByte +
                nibble                                            -- Manually set the most recent byte to the lastByte + the front part of `n`
            byteCount = byteCount + 1
    ...
    local function writeUnsigned(width, n)
        ...
        if width <= 32 then
            ...
            writeBits(unpack(extractedBits))
        else
            -- If the width is greater than 32, the number has to be divided up into a few 32-bit or less numbers
            local leastSignificantChunk = n %
                0x100000000                                          -- Get bits 0-31 (counting from the right side). 0x100000000 is 2^32.
            local mostSignificantChunk = math.floor(n / 0x100000000) -- Get any remaining bits by manually right shifting by 32 bits

            local c = width -
                32                     -- The number of bits in mostSignificantChunk is variable, but a counter is still needed
    ...
    local function writeField(...)
        ...
        for i = 1, bools.n do
            field = field *
                2 -- Shift `field`. Equivalent to field<<1. At the beginning of the loop to avoid an extra shift.

            local v = bools[i]
            if v then
                field = field +
                    1 -- If the bit is truthy, turn it on (it defaults to off so it's fine to not have a branch)
            end
        end
    ...
    local function writeUInt16(n)
        ...
        local b1, b2 = bit32.rshift(n, 8), bit32.band(n, 255)
        if opts.little then
            writeByte(b2)
            writeByte(b1)
        else
    ...
    local function writeUInt32(n)
        ...
        if opts.little then n = bit32.byteswap(n) end
    ...
    local function readBits(n)
        ...
        local output = {}     --!
        local byte = bytes
            [pointerByte]     -- For the sake of efficiency, the current byte that the bits are coming from is stored
    ...
    local function readString()
        ...
        for i = 1, stringLength, 4096 do
            output[k] = string.char(unpack(outputCharacters, i, math.min(stringLength, i + 4095)))
            k = k + 1
        end
    ...
    local function readTerminatedString()
        ...
        for l = 1, length, 4096 do
            output[k] = string.char(unpack(outputCharacters, l, math.min(length, l + 4095)))
            k = k + 1
        end
    ...
    local function readSetLengthString(length)
        ...
        for i = 1, length, 4096 do
            output[k] = string.char(unpack(outputCharacters, i, math.min(length, i + 4095)))
            k = k + 1
        end
    ...
    local function readUInt16()
        ...
        return opts.little and (bit32.lshift(b2, 8) + b1) or (bit32.lshift(b1, 8) + b2)
    end
    ...
    local function readUInt32()
        ...
        return opts.little and bit32.byteswap(n) or n
    end
